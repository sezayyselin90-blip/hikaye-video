#!/usr/bin/env python3
"""
TTS Ses Üretici: Türkçe senaryoyu ElevenLabs ile seslendiriyor
"""
...
def generate_tts_audio(text: str, voice_id: str, output_path: str) -> bool:
    """ElevenLabs TTS ile ses dosyası oluştur"""
    api_key = os.getenv("ELEVENLABS_API_KEY")

    if not api_key:
        print("  ⚠️  ElevenLabs API anahtarı bulunamadı")
        return False
...
def main():
    ...
    print("🎙️ ElevenLabs TTS ile ses dosyaları oluşturuluyor...")
    ...
    for scene in scenes:
        ...
        print(f"[SAHNE {scene_id:02d}] {text[:50]}...", end=' ')
    ...
    print(f"\n✓ {success_count}/{len(scenes)} ses dosyası oluşturuldu")

    # Senaryo ve ses mapping'i kaydet
    metadata = {
        "voice_id": voice_id,
    ...
    with open("metadata_audio.json", 'w', encoding='utf-8') as f:
        json.dump(metadata, f, ensure_ascii=False, indent=2)

    print("\n" + "=" * 80)
    print("✅ SES DOSYALARI HAZIR!")
    print("=" * 80)
    print(f"\n🎵 Üretilen dosyalar: {audio_dir}/")
    print(f"📋 Metadata: metadata_audio.json")
    print("\nSonraki adım: Görsel asset üretimi...")
    print("=" * 80 + "\n")
